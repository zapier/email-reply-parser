...
class EmailMessageTest(unittest.TestCase):
    ...
    def test_simple_body(self):
        ...
        self.assertEquals([False, True], map(lambda x: x, [f.signature for f in message.fragments]))
        self.assertEquals([False, True], map(lambda x: x, [f.hidden for f in message.fragments]))
